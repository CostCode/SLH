...
import edu.cmu.cc.slh.view.validator.textview.MoneyValidator;
import edu.cmu.cc.slh.view.validator.textview.NameValidator;
import edu.cmu.cc.slh.view.validator.textview.QuantityValidator;
...
public class SLItemViewAdapter extends AbstractViewAdapter {
	...
	public static void updateView(View view) {
		...
		WidgetUtils.getEditText(view, R.id.et_sl_item_name)
		...
		WidgetUtils.getEditText(view, R.id.et_sl_item_quantity)
		...
		if (item.getPrice() != null) {
			WidgetUtils.getEditText(view, R.id.et_sl_item_price)
				.setText(StringUtils
					.getMoneyFormatString(item.getPrice().doubleValue()));
		}
		...
		WidgetUtils.getEditText(view, R.id.et_sl_item_comment)
		...
		Spinner unitsSpinner = 
				WidgetUtils.getSpinner(view, R.id.sp_sl_item_unit);
		
		String unit = ShoppingListItem.Unit.getUnitNameByCode(item.getUnit());
	...
	public static void updateModel(View view) {
		...
		String strQuantity = 
				WidgetUtils.getEditTextAsString(view, R.id.et_sl_item_quantity);
		...
		item.setDescription(WidgetUtils
				.getEditTextAsString(view, R.id.et_sl_item_comment));
		...
		if (selectedUnit instanceof String) {
			int unitCode = ShoppingListItem.Unit
					.getUnitCodeByName((String)selectedUnit);
	...
	private void initializeValidators(View parentView) {
		...
		synchronized (validatingViews) {
			...
			assignValidatorToView(parentView, R.id.et_sl_item_quantity, 
					R.string.sl_item_quantity, 
					new QuantityValidator(ctx));
			...
			assignValidatorToView(parentView, R.id.et_sl_item_price, 
					R.string.sl_item_price, 
					new MoneyValidator(ctx));
			
			assignValidatorToView(parentView, R.id.et_sl_item_comment, 
					R.string.sl_item_comment, 
					new CommentValidator(ctx));
		}
	}
